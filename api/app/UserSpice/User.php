<?php
namespace App\UserSpice;

use App\UserSpice\Auth\Authenticatable;
use Config;
...
class User implements AuthenticatableContract, AuthorizableContract, UserEntityInterface, UserRepositoryInterface
{
	...
	public function __construct($user = null){
		...
		$this->_sessionName = Config::get('userspice.session.session_name');
		$this->_cookieName = Config::get('userspice.remember.cookie_name');
	...
	public function find($user = null,$loginHandler = null){
		...
		if ($user) {
			...
			if ($data->count()) {
                ...
                $permissions = $this->_db->query("select p.id, p.name from permissions p, user_permission_matches m where p.id = m.permission_id and m.user_id = ?", [$this->_data->id]);
                foreach($permissions->results() as $p) {
                    $this->_data->permissions[] = [
                        'id' => $p->id,
                        'name' => $p->name,
                    ];
	...
	public function login($username = null, $password = null, $remember = false){
        require_once(__DIR__.'/../../../users/helpers/users_online.php');
			...
			if ($user) {
				if (password_verify($password,$this->data()->password)) {
					...
					if ($remember) {
						...
						Cookie::put($this->_cookieName, $hash, Config::get('userspice.remember.cookie_expiry'));
					}
	...
	public function hasPermission($name){
		$permissions = $this->_db->query('select p.name from permissions p, user_permission_matches m where p.id = m.permission_id and m.user_id = ?', [$this->data()->id]);
        foreach($permissions->results() as $p) {
            if($p->name == $name) {
                return true;
            }
    ...
    {
        return $this->login($username, $password) ? $this : null;
    }
}
