...
export default class CuratorDesignList extends Component {
    ...
    onRefresh(e) {
        ...
        handleRefresh(ListType.CURATOR);
    }
    ...
    onCopy(e, id)  {
        e.preventDefault();
        const {handleCuratedCopy} = this.props;
        handleCuratedCopy(id);
    }

    onDelete(e, id)  {
        e.preventDefault();
        const {handleCuratedDelete, getAlertify} = this.props;
        ...
        alertify.confirm({
            ...
            confirmAction: () => handleCuratedDelete(id),
            title: 'DELETE DESIGN',
    ...
    renderSettings() {
        const {configLoadedNum, editCourseId, acls, data} = this.props.list;
        const {loadDesignList, resetSettings, loadDesignConfig, configureDesign, getAlertify} = this.props;
        const list = {
            configLoadedNum,
            editId: editCourseId,
            acls,
        ...
        let props = {
            displayShareAll: true,
            title,
            list,
            refreshList: () => loadDesignList(ListType.CURATOR),
            resetSettings,
            loadConfig: loadDesignConfig,
            configure: configureDesign,
            getAlertify,
        };
        return <Settings ref={modal => this.settingsModal = modal} {...props} list={list}/>;
    }
